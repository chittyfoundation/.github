...
# ---------------------------------------------------------------------------
# Checkout action version pinning
# ---------------------------------------------------------------------------


class TestCheckoutVersionPinning:
    """All jobs must pin actions/checkout to @v4 (not a floating tag)."""

    @pytest.mark.parametrize("job_id", ["detect", "lint", "test", "security", "ai-review"])
    def test_checkout_pinned_to_v4(self, workflow, job_id):
        """Each job's checkout step must use exactly actions/checkout@v4."""
        job = workflow["jobs"][job_id]
        checkout_steps = [
            s for s in job["steps"] if "actions/checkout" in s.get("uses", "")
        ]
        assert checkout_steps, f"Job '{job_id}' has no checkout step"
        for step in checkout_steps:
            assert step["uses"] == "actions/checkout@v4", (
                f"Job '{job_id}' uses '{step['uses']}' instead of 'actions/checkout@v4'"
            )

    @pytest.mark.parametrize("job_id", ["detect", "lint", "test", "security", "ai-review"])
    def test_checkout_is_first_step(self, workflow, job_id):
        """Checkout should be the very first step in every job."""
        steps = workflow["jobs"][job_id]["steps"]
        assert "actions/checkout" in steps[0].get("uses", ""), (
            f"Job '{job_id}' does not have checkout as its first step"
        )


# ---------------------------------------------------------------------------
# test job – additional coverage
# ---------------------------------------------------------------------------


class TestTestJobExtra:
    @pytest.fixture
    def test_job(self, workflow):
        return workflow["jobs"]["test"]

    def test_node_setup_uses_node_version_input(self, test_job):
        """The test job's setup-node step must reference inputs.node-version."""
        setup_step = next(
            (s for s in test_job["steps"] if "setup-node" in s.get("uses", "")), None
        )
        assert setup_step is not None
    ...
    def test_test_run_uses_working_directory_input(self, test_job):
        """The test job's npm run step must reference inputs.working-directory."""
        run_step = next(
            (s for s in test_job["steps"] if "npm ci" in str(s.get("run", ""))), None
        )
        assert run_step is not None
    ...
    def test_npm_test_uses_if_present_flag(self, test_job):
        """npm test should use --if-present so missing script doesn't fail."""
        run_step = next(
            (s for s in test_job["steps"] if "npm ci" in str(s.get("run", ""))), None
        )
        assert run_step is not None
...
# ---------------------------------------------------------------------------
# security job – additional coverage
# ---------------------------------------------------------------------------


class TestSecurityJobExtra:
    @pytest.fixture
    def security(self, workflow):
        return workflow["jobs"]["security"]

    def test_npm_audit_uses_working_directory_input(self, security):
        """npm audit step must reference inputs.working-directory."""
        audit_step = next(
            (s for s in security["steps"] if "npm audit" in str(s.get("run", ""))),
            None,
        )
        assert audit_step is not None
        assert "inputs.working-directory" in str(audit_step.get("working-directory", ""))

    def test_snyk_action_ref_is_node_master(self, security):
        """Snyk action must use exactly snyk/actions/node@master."""
        snyk_step = next(
            (s for s in security["steps"] if "snyk" in s.get("uses", "").lower()),
            None,
        )
        assert snyk_step is not None
        assert snyk_step["uses"] == "snyk/actions/node@master"

    def test_snyk_token_env_references_secret(self, security):
        """SNYK_TOKEN env in snyk step must reference secrets.SNYK_TOKEN."""
        snyk_step = next(
            (s for s in security["steps"] if "snyk" in s.get("uses", "").lower()),
            None,
        )
        assert snyk_step is not None
        env_val = str(snyk_step.get("env", {}).get("SNYK_TOKEN", ""))
        assert "secrets.SNYK_TOKEN" in env_val


# ---------------------------------------------------------------------------
# DETECT_SCRIPT matches the workflow's embedded script
# ---------------------------------------------------------------------------


class TestDetectScriptConsistency:
    """The DETECT_SCRIPT constant used in shell tests must match the script
    actually embedded in the workflow YAML."""

    def test_detect_script_constant_matches_workflow(self, workflow):
        """DETECT_SCRIPT must be a subset of the script declared in the workflow."""
        detect_step = next(
            s
            for s in workflow["jobs"]["detect"]["steps"]
            if s.get("id") == "detect"
        )
        workflow_script = detect_step["run"]
        # Each non-blank line of the DETECT_SCRIPT constant must appear verbatim
        # in the workflow script (modulo leading/trailing whitespace per line).
        canonical_lines = [
            l.strip() for l in DETECT_SCRIPT.splitlines() if l.strip()
        ]
        workflow_lines = [l.strip() for l in workflow_script.splitlines() if l.strip()]
        for line in canonical_lines:
            assert line in workflow_lines, (
                f"DETECT_SCRIPT line '{line}' not found in workflow script"
            )


# ---------------------------------------------------------------------------
# detect script – additional edge-case scenarios
# ---------------------------------------------------------------------------


class TestDetectScriptEdgeCases:
    def test_all_indicator_files_present_picks_node(self, tmp_path):
        """When all indicator files exist, node has highest priority."""
        lang = _run_detect_script(
            str(tmp_path),
    ...
    def test_only_go_mod_detects_go(self, tmp_path):
        """A directory with only go.mod must be detected as go."""
        lang = _run_detect_script(str(tmp_path), ["go.mod"])
        assert lang == "go"

    def test_requirements_txt_case_sensitive(self, tmp_path):
        """Requirements.txt (capital R) should not be detected as python."""
        lang = _run_detect_script(str(tmp_path), ["Requirements.txt"])
        assert lang == "unknown"

    def test_pyproject_toml_case_sensitive(self, tmp_path):
        """Pyproject.toml (capital P) should not be detected as python."""
        lang = _run_detect_script(str(tmp_path), ["Pyproject.toml"])
        assert lang == "unknown"

    def test_detect_output_file_is_created(self, tmp_path):
        """Script must create the GITHUB_OUTPUT file even for unknown repos."""
        output_file = os.path.join(str(tmp_path), "github_output")
        env = {**os.environ, "GITHUB_OUTPUT": output_file}
        result = subprocess.run(
            ["bash", "-c", DETECT_SCRIPT],
        ...
        assert os.path.exists(output_file), "GITHUB_OUTPUT file was not created"

    def test_script_exits_zero_for_every_known_language(self, tmp_path):
        """Script must exit 0 for each recognised project type."""
        for files in [
            ["package.json"],
            ["requirements.txt"],
            ["pyproject.toml"],
            ...
            subdir = tmp_path / "_".join(files)
            subdir.mkdir()
            output_file = str(subdir / "out")
            env = {**os.environ, "GITHUB_OUTPUT": output_file}
            for fname in files:
                (subdir / fname).touch()
            result = subprocess.run(
            ...
            assert result.returncode == 0, (
                f"Script failed for {files}: {result.stderr}"
            )


# ---------------------------------------------------------------------------
# lint job – npm run lint --if-present flag
# ---------------------------------------------------------------------------


class TestLintJobExtra:
    @pytest.fixture
    def lint(self, workflow):
        return workflow["jobs"]["lint"]

    def test_npm_run_lint_uses_if_present_flag(self, lint):
        """npm run lint must use --if-present so missing script is non-fatal."""
        run_step = next(
            (s for s in lint["steps"] if "npm ci" in str(s.get("run", ""))), None
        )
        assert run_step is not None
...
# ---------------------------------------------------------------------------
# ai-review job – anthropic action version is beta
# ---------------------------------------------------------------------------


class TestAiReviewJobExtra:
    @pytest.fixture
    def ai_review(self, workflow):
        return workflow["jobs"]["ai-review"]

    def test_claude_action_uses_beta_ref(self, ai_review):
        """claude-code-action should pin to the @beta ref."""
        claude_step = next(
            (s for s in ai_review["steps"] if "claude-code-action" in s.get("uses", "")),
            None,
        )
        assert claude_step is not None
        assert claude_step["uses"] == "anthropics/claude-code-action@beta"

    def test_ai_review_exactly_two_permissions(self, ai_review):
        """ai-review should declare exactly two permission entries."""
        perms = ai_review.get("permissions", {})
        assert len(perms) == 2

    def test_condition_uses_github_event_name_expression(self, ai_review):
        """Condition must reference github.event_name, not a hardcoded string."""
        condition = str(ai_review.get("if", ""))
